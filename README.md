...
Кроме матчей Valorant, разбираются и **клипы аим-тренажёров** (KovaaK's, Aimbeast):
лог прогона KovaaK's приносит то, чего нет в кадре, — моменты попаданий и TTK.
...
Тот же движок метрик, другой вход: клип из KovaaK's или Aimbeast
(`training_platform=kovaaks|aimbeast` на форме загрузки).

```mermaid
flowchart LR
    V[Клип тренажёра] --> D[Цветовой детектор сфер]
    D --> E[Эпизоды и метрики<br/>в тех же HU]
    L[Лог прогона CSV] --> P[Парсер: килы, TTK,<br/>shots/hits]
    E --> S[Синхронизация<br/>лог ↔ видео]
    P --> S
    S --> R[Секция попаданий<br/>в evidence-JSON]
```

- **Детектор — не YOLO.** Цель тренажёра — однотонная сфера на нейтральном фоне,
  поэтому её находит HSV-сегментация (`backend/trainer_target_detector.py`,
  пресет цвета — `TRAINER_TARGET_COLOUR`). Диаметр сферы играет роль высоты головы,
  и HU-нормировка работает без изменений.
- **Лог прогона — готовый эталон.** KovaaK's пишет `… Stats.csv` с каждым килом,
  его временем и TTK, плюс сводку Shots/Hits. Парсер (`engine/trainer_stats/`)
  проверен на реальных логах 2.x из открытых источников.
- **Синхронизация без участия игрока.** Запись экрана и прогон стартуют независимо.
  Сдвиг выводится автоматически: моменты, когда трек цели пропал на видео,
  совмещаются с килами из лога (`engine/trainer_stats/sync.py`). Если совпадений
  мало, лог считается чужим и не приписывается клипу — лучше остаться без секции
  попаданий, чем показать чужую статистику.
- **Происхождение чисел видно.** Секция помечена `measured_by: trainer_log`:
  валидатор коуча отличает числа тренажёра от измеренных движком.

Лог необязателен: без него клип тренажёра разбирается как чистый CV-путь.
Aimbeast моменты выстрелов не пишет (только итоги прогонов), поэтому для него
секция попаданий пока недоступна.
...
| `TRAINER_TARGET_COLOUR` | `orange` | Цвет целей тренажёра: `orange`, `red`, `green`, `purple`, `blue` |
| `MAX_STATS_MB` | `5` | Лимит размера лога прогона тренажёра |
...
574 теста backend и 18 frontend. Тесты с маркером `integration` трогают реальные
файлы датасета; остальные работают без сети, GPU и внешних ключей — провайдеры и
хранилище инжектируются, а переменные из локального `.env` на тесты не влияют.
...
### Внешний ранг KovaaK's как ограничитель плана

Если игрок указал SteamID64, пайплайн берёт его прогресс по бенчмаркам Voltaic S5
из API KovaaK's (`backend/services/kovaaks_client.py`). API недокументирован,
поэтому снапшот кэшируется на час, а любой сбой деградирует в «нет данных» без
падения разбора. Ранг попадает в evidence-JSON (schema 1.4) и ограничивает тир
KovaaK's-дриллов: следующий тир открыт, только если его сценарий уже играется или
сценарий предыдущего тира доведён до верхнего порога. SteamID в отчёт и в
share-выдачу не утекает — это закреплено тестом.
...
- Модель обучена на голову врага в Valorant; на другие игры не переносится.
  Для аим-тренажёров используется отдельный цветовой детектор.
...
- В клипах Valorant выстрелов в данных нет: момент попадания не измеряется,
  метрики опираются на видимую геометрию. Для KovaaK's попадания и TTK даёт лог
  прогона; Aimbeast таких данных не пишет.
- Режим тренажёра проверен на синтетике и реальных логах KovaaK's; пороги
  метрик для тренажёра пока совпадают с валорантовыми и ждут калибровки на
  реальных клипах.

## Статус

Рабочий прототип: пайплайн end-to-end, 574 + 18 тестов, замеренная юнит-экономика
